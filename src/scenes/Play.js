...
export class PlayScene extends Phaser.Scene {
    ...
    trackables;
    RESOURCE_GOAL = 1000;
    startingResources = 100; // Default value
    buildings = [];
    gameState;
    grid;
    stats;
    ...
    init() {
        ...
        this.buildings = config.buildings.map((building) => ({
            ...building,
    ...
    create() {
        ...
        this.stats = new Stats(this, this.statsConfig.x, this.statsConfig.y, this.statsConfig.width, this.statsConfig.height);
    ...
    updateVictoryConditionText() {
        ...
        if (victoryConditionElement) {
            if (this.victoryCondition.type === "resources") {
                ...
                victoryConditionElement.textContent = LanguageManager.getTranslation("specificBuildingGoal", {
                    goal: this.victoryCondition.goal,
                    buildingType: LanguageManager.getTranslation(this.victoryCondition.buildingType ?? ""),
                });
            }
        }
    }
    ...
    updateUIText() {
        ...
        if (playerDisplay) {
            playerDisplay.innerHTML =
                `<span data-translate="resources">${LanguageManager.getTranslation("resources")}</span>: ${this.player.resources}<br />` +
                    `<span data-translate="turns">${LanguageManager.getTranslation("turns")}</span>: ${this.trackables.turnsPlayed}<br />` +
                    `<span data-translate="buildingsPlaced">${LanguageManager.getTranslation("buildingsPlaced")}</span>: ${this.trackables.buildingsPlaced}`;
        }
        // Update stats UI
    ...
    applyWeatherCondition() {
        if (this.weatherCondition) {
            ...
            if (currentTurn >= this.weatherCondition.startTurn &&
    ...
    checkWinCondition() {
        if (this.victoryCondition.type === "resources") {
            ...
            const level3Buildings = Array.from(this.grid.cells.values()).filter((cell) => cell.level === 3).length;
            if (level3Buildings >= this.victoryCondition.goal) {
                this.scene.start("sceneWin", this.trackables);
            }
        }
        else if (this.victoryCondition.type === "specificBuilding") {
            const specificBuildings = Array.from(this.grid.cells.values()).filter((cell) => cell.getName() === this.victoryCondition.buildingType).length;
